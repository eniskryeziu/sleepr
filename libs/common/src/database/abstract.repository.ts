...
export abstract class AbstractRepository<TDocument extends AbstractDocument> {
    ...
    async findOne(filterQuery: QueryFilter<TDocument>): Promise<TDocument> {
        ...
        if (!document) {
            this.logger.warn("(FIND ONE)Document was not found with filterQuery", filterQuery);
    ...
    async findOneAndUpdate(filterQuery: QueryFilter<TDocument>, update: UpdateQuery<TDocument>): Promise<TDocument> {
        ...
        if (!document) {
            this.logger.warn("(FIND ONE AND UPDATE)Document was not found with filterQuery", filterQuery);
            throw new NotFoundException("Document not found");
        }
        return document;
    }
